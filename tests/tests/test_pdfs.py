...
This module tests whether generated PDF documents meet specified quality criteria
including page count, size, metadata, and content requirements.

The tests use a data-driven approach where the configuration is loaded from config.yml
and applied to all generated PDF files. This differs from traditional unit testing
where functions are tested with generated sample data.

Idea from: https://blog.martisak.se/2020/05/16/latex-test-cases/

Configuration:
    Tests are configured via tests/config.yml which defines:
    - Page requirements (count, size)
    - File size bounds
...
from tests.utils import _PROJECT_ROOT, fix_tzoffset, parse_duration, parse_size
...
def get_project_root_files(suffix: str) -> List[Path]:
    ...
    """
    # Look for files in the build directory
    build_dir = _PROJECT_ROOT / "build"
    if build_dir.exists():
        yield from build_dir.glob("*." + suffix)
