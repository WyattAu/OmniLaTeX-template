...
EXPECTED_INSTITUTIONS = [
    "generic", "tum", "eth", "mit", "stanford", "cambridge",
    "tuhh", "harvard", "oxford", "princeton",
    "columbia", "yale", "imperial", "cmu", "epfl", "tudelft",
]
...
class TestInstitutionStructure:
    ...
    def test_expected_institutions_present(self):
        ...
        for name in EXPECTED_INSTITUTIONS:
            assert name in actual, f"Institution '{name}' not found in {INSTITUTIONS_DIR}"
        ...
        for d in INSTITUTIONS_DIR.iterdir():
            if d.is_dir():
                for sty in d.glob("*.sty"):
                    ...
                    assert len(content) > 50, f"{sty} is suspiciously short ({len(content)} chars)"
    ...
    def test_generic_is_not_production(self):
        ...
        assert generic_sty.is_file(), "Generic template must exist for scaffold-institution"
