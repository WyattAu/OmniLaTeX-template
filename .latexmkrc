...
# However, `make` can also do much more, e.g. cover clean-up operations etc.
...
# or 'prod' for production builds with full validation
# Default is 'dev' for faster iteration

my $build_mode = $ENV{'BUILD_MODE'} || 'dev';
my $do_gls = 1;

if (exists $ENV{'OMNILATEX_SKIP_BIB2GLS'} && $ENV{'OMNILATEX_SKIP_BIB2GLS'} eq '1') {
    $do_gls = 0;
}

if ($build_mode eq 'prod') {
    # Production mode: thorough compilation
    $max_repeat = 7;  # More iterations for complex documents
} elsif ($build_mode eq 'ultra') {
    # Ultra-lite mode: single pass, skip heavyweight helpers
    $max_repeat = 1;
    $pdf_mode = 4;
...
# Validation enabled in production mode only for performance
if ($build_mode eq 'prod') {
...
# (.dat, .script, .table).
# Also clean minted cache directories
$clean_ext = "%R-*.glstex %R_contourtmp*.* _minted-%R";

# ======================================================================================
# Minted Cache Optimization
# ======================================================================================
# Exclude minted cache from dependency tracking for better performance
# This prevents latexmk from re-running when only cache files change
$hash_calc_ignore_pattern{'_minted'} = '.*';
