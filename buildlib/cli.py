from __future__ import annotations

import argparse
import os
import sys

from buildlib.config import ProjectConfig
from buildlib.runner import CommandRunner
from buildlib.tasks import BuildTasks
from buildlib.tui import interactive_menu
from buildlib.ui import TerminalOutput

# ---------------------------------------------------------------------------
# Command registry
# ---------------------------------------------------------------------------

_COMMANDS: dict[str, tuple] = {
    "build": (BuildTasks.build_all, "Build root and all examples.", False),
    "build-root": (BuildTasks.build_root, "Build root document.", False),
    "build-all": (BuildTasks.build_all, "Alias for 'build'.", False),
    "clean": (BuildTasks.clean_all, "Full cleanup.", False),
    "clean-aux": (BuildTasks.clean_aux, "Clean aux files.", False),
    "clean-pdf": (BuildTasks.clean_pdf, "Clean all PDFs.", False),
    "preflight": (BuildTasks.preflight, "Run preflight checks.", False),
    "check": (
        BuildTasks.cmd_check,
        "Cross-reference integrity check on .tex files.",
        True,
    ),
    "lint": (BuildTasks.cmd_lint, "Lint .tex files with chktex/lacheck.", True),
    "list-examples": (BuildTasks.list_examples, "List all examples.", False),
    "build-example": (
        BuildTasks.build_example,
        "Build specific example(s) concurrently.",
        True,
    ),
    "build-examples": (
        BuildTasks.build_examples,
        "Build all examples concurrently.",
        False,
    ),
    "clean-example": (BuildTasks.clean_example, "Clean specific example(s).", True),
    "clean-examples": (BuildTasks.clean_aux, "Clean all examples.", False),
    "cache-stats": (
        BuildTasks.cmd_cache_stats,
        "Show build cache statistics.",
        False,
    ),
    "cache-clear": (BuildTasks.cmd_cache_clear, "Delete build cache.", False),
    "test": (BuildTasks.run_tests, "Run test suite.", True),
    "watch": (BuildTasks.cmd_watch, "Watch files for changes and rebuild.", True),
    "doctor": (
        BuildTasks.cmd_doctor,
        "Comprehensive environment health diagnostics.",
        False,
    ),
    "diff": (
        BuildTasks.cmd_diff,
        "Compare PDFs against references for visual regression.",
        True,
    ),
    "scaffold-institution": (
        BuildTasks.cmd_scaffold_institution,
        "Create a new institution config from the generic template.",
        True,
    ),
    "scaffold-language": (
        BuildTasks.cmd_scaffold_language,
        "Create a language addition guide with translation stubs.",
        True,
    ),
    "init": (
        BuildTasks.cmd_init,
        "Initialize a new OmniLaTeX project from a template.",
        True,
    ),
    "export": (None, "Export LaTeX to HTML, EPUB, DOCX, or Markdown.", True),
    "plugin-list": (
        BuildTasks.cmd_plugin_list,
        "List installed plugins.",
        False,
    ),
    "plugin-search": (None, "Search the plugin registry.", False),
    "plugin-install": (None, "Install a plugin from the registry.", False),
    "plugin-remove": (None, "Remove an installed plugin.", False),
    "plugin-validate": (
        BuildTasks.cmd_plugin_validate,
        "Validate all plugin manifests.",
        False,
    ),
    "plugin-info": (None, "Show detailed plugin information.", False),
}

# Commands that need extra arguments passed via lambda (require args object).
_LAMBDA_COMMANDS = frozenset(
    {"export", "plugin-search", "plugin-install", "plugin-remove", "plugin-info"}
)


# ---------------------------------------------------------------------------
# Parser construction
# ---------------------------------------------------------------------------


def _create_parser(default_jobs: int) -> argparse.ArgumentParser:
    """Build the full argument parser with all subcommands."""
    parser = argparse.ArgumentParser(
        description="OmniLaTeX build tool.",
        formatter_class=argparse.RawTextHelpFormatter,
    )
    parser.add_argument(
        "--mode", choices=["dev", "prod", "ultra"], default="dev", help="Build mode."
    )
    parser.add_argument(
        "--verbose", action="store_true", help="Enable verbose log output."
    )
    parser.add_argument(
        "--timings",
        action="store_true",
        help="Record per-example build metrics to build/metrics.json.",
    )
    parser.add_argument(
        "--force",
        action="store_true",
        help="Force rebuild, ignoring incremental build cache.",
    )
    parser.add_argument(
        "--source-date-epoch",
        type=int,
        default=None,
        metavar="TIMESTAMP",
        help="Set SOURCE_DATE_EPOCH for reproducible builds (Unix timestamp).",
    )
    parser.add_argument(
        "-j",
        "--jobs",
        type=int,
        default=default_jobs,
        help=f"Parallel jobs. Default: {default_jobs}",
    )
    parser.add_argument(
        "--cnf-line",
        action="append",
        default=[],
        metavar="LINE",
        help="Extra TeX configuration line passed to lualatex via --cnf-line (can be repeated).",
    )

    subparsers = parser.add_subparsers(dest="command")
    subparser_map: dict[str, argparse.ArgumentParser] = {}

    for name, (handler, help_text, takes_files) in _COMMANDS.items():
        sub = subparsers.add_parser(name, help=help_text)
        sub.set_defaults(handler=handler)
        if takes_files:
            sub.add_argument("files", nargs="*", default=None)
        subparser_map[name] = sub

        # Plugin subcommand arguments
        if name in ("plugin-install", "plugin-remove", "plugin-info"):
            sub.add_argument("plugin_name", type=str, help="Plugin name.")
        if name == "plugin-search":
            sub.add_argument(
                "plugin_query",
                type=str,
                nargs="?",
                default="",
                help="Search query.",
            )

    # diff-specific arguments
    subparser_map["diff"].add_argument(
        "--regenerate-references",
        action="store_true",
        dest="regenerate_references",
        default=False,
        help="Copy built PDFs to tests/references/ as new baselines instead of comparing.",
    )
    subparser_map["diff"].add_argument(
        "--output",
        "-o",
        type=str,
        default=None,
        metavar="PATH",
        help="Output path for annotated diff PDF (used with two-PDF or git-ref mode).",
    )

    # init-specific arguments
    subparser_map["init"].add_argument(
        "--doctype",
        type=str,
        default=None,
        help="Document type (e.g. book, thesis, article, poster, presentation, letter)",
    )
    subparser_map["init"].add_argument(
        "--institution",
        type=str,
        default=None,
        help="Institution config name (e.g. tum, eth, none)",
    )
    subparser_map["init"].add_argument(
        "--language",
        type=str,
        default=None,
        help="Document language (e.g. english, german, chinese)",
    )
    subparser_map["init"].add_argument(
        "--thesis",
        action="store_true",
        default=False,
        help="Shortcut: set doctype=thesis and create full thesis project structure.",
    )

    # export-specific arguments
    subparser_map["export"].add_argument(
        "--format",
        "-f",
        dest="export_format",
        type=str,
        default="html",
        choices=["html", "html5", "epub", "epub3", "docx", "md"],
        help="Output format (default: html)",
    )

    # list-examples-specific arguments
    subparser_map["list-examples"].add_argument(
        "--format",
        "-f",
        dest="list_format",
        type=str,
        default="text",
        choices=["text", "json"],
        help="Output format (default: text)",
    )

    return parser


# ---------------------------------------------------------------------------
# Command dispatch
# ---------------------------------------------------------------------------


def _dispatch_command(
    args: argparse.Namespace,
    tasks: BuildTasks,
    ui: TerminalOutput,
) -> None:
    """Route the parsed arguments to the correct handler method."""
    cmd = args.command
    handler = args.handler
    files = getattr(args, "files", None)

    try:
        if cmd == "init":
            handler(
                tasks,
                files,
                doctype=getattr(args, "doctype", None),
                institution=getattr(args, "institution", None),
                language=getattr(args, "language", None),
                thesis=getattr(args, "thesis", False),
            )
        elif cmd == "list-examples":
            handler(tasks, files, output_format=getattr(args, "list_format", "text"))
        elif cmd == "diff":
            handler(
                tasks,
                files,
                regenerate_references=getattr(args, "regenerate_references", False),
                output=getattr(args, "output", None),
            )
        elif cmd in ("lint", "check"):
            rc = handler(tasks, files)
            if rc:
                sys.exit(rc)
        elif cmd == "export":
            handler(tasks, files, output_format=getattr(args, "export_format", "html"))
        elif cmd == "plugin-search":
            tasks.cmd_plugin_search(files, query=getattr(args, "plugin_query", ""))
        elif cmd == "plugin-install":
            tasks.cmd_plugin_install(files, name=getattr(args, "plugin_name", ""))
        elif cmd == "plugin-remove":
            tasks.cmd_plugin_remove(files, name=getattr(args, "plugin_name", ""))
        elif cmd == "plugin-info":
            tasks.cmd_plugin_info(files, name=getattr(args, "plugin_name", ""))
        else:
            handler(tasks, files)
    except Exception as e:
        ui.error(f"An unexpected top-level error occurred: {e}")
        if os.environ.get("OMNILATEX_VERBOSE") or os.environ.get("CI"):
            import traceback

            traceback.print_exc()
        sys.exit(1)


# ---------------------------------------------------------------------------
# Entry point
# ---------------------------------------------------------------------------


def main() -> None:
    ui, config = TerminalOutput(), ProjectConfig()
    default_jobs = 4 if config.is_ci() else (os.cpu_count() or 4)

    parser = _create_parser(default_jobs)
    args = parser.parse_args()

    if args.source_date_epoch is not None:
        os.environ["SOURCE_DATE_EPOCH"] = str(args.source_date_epoch)
    config.cnf_lines = args.cnf_line
    if args.command and "build" in args.command:
        ui.info(f"Using up to {args.jobs} parallel jobs.")

    runner = CommandRunner(
        ui, build_mode=args.mode, verbose=(args.verbose or config.verbose_enabled())
    )
    tasks = BuildTasks(
        config, runner, ui, jobs=args.jobs, timings=args.timings, force=args.force
    )

    # No command given — show interactive menu (TTY) or help (CI)
    if not args.command:
        if config.is_ci() or not sys.stdout.isatty():
            parser.print_help()
            return
        interactive_menu(tasks, _COMMANDS)
        return

    _dispatch_command(args, tasks, ui)
