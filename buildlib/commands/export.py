...
class ExportMixin:
    ...
    def cmd_export(self, files: list[str] | None = None, output_format: str = "html"):
        ...
        source = self._resolve_source(files, repo_root)
        if source is None:
            return
        ...
        dispatch = {
            ...
            "html5": self._export_html5,
            "epub": self._export_epub,
            "epub3": self._export_epub,
            "docx": self._export_docx_md,
            "md": self._export_docx_md,
            "markdown": self._export_docx_md,
        }

        handler = dispatch.get(output_format)
        if handler:
            handler(source, output_dir, output_format)
        else:
            self.ui.error(
    ...
    def _resolve_source(self, files: list[str] | None, repo_root: Path) -> Path | None:
        """Resolve the source .tex file from user input."""
        if files:
            source = Path(files[0])
    ...
    def _export_html(self, source: Path, output_dir: Path, _fmt: str):
        """High-fidelity HTML via LaTeXML."""
        if not self._check_tool("latexml"):
    ...
    def _export_html5(self, source: Path, output_dir: Path, _fmt: str):
        """Lightweight HTML5 via pandoc."""
        if not self._check_tool("pandoc"):
    ...
    def _export_epub(self, source: Path, output_dir: Path, output_format: str):
        ...
        meta_file = _cfg.REPO_ROOT / "export" / "metadata.xml"
        if meta_file.exists():
            epub_args.insert(-2, f"--epub-metadata={meta_file}")

        # Inject CSS if export/epub.css exists
        css_file = _cfg.REPO_ROOT / "export" / "epub.css"
        if css_file.exists():
    ...
    def _export_docx_md(self, source: Path, output_dir: Path, output_format: str):
