...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}
        ...
        % Intersection labels
        \node[font=\scriptsize] at (\cx + 0.5, \cy + 1.3) {Quantum\\Computing};
        \node[font=\scriptsize] at (\cx + \cx/1.2 - 0.5, \cy + 1.3) {Numerical\\Analysis};
        \node[font=\scriptsize] at (\cx + \cx/2.4, \cy + 0.5) {Machine\\Learning};
        \node[font=\scriptsize] at (\cx + \cx/2.4, \cy + 1.3) {Theory};
    \end{tikzpicture}
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}
        ...
        \foreach \row/\data/\y in {
            ...
            \foreach \col/\val [count=\i from 0] in \data {
                \pgfmathparse{\val}
                \pgfmathsetmacro{\intensity}{abs(\pgfmathresult)}
                \pgfmathsetmacro{\redcolor}{100*\pgfmathresult + 155}
                \pgfmathsetmacro{\bluecolor}{-100*\pgfmathresult + 155}
                \ifdim \pgfmathresult pt > 0pt
                    \definecolor{cellcolor}{rgb}{1,1-\intensity,1-\intensity}
                \else
                    \definecolor{cellcolor}{rgb}{1-\intensity,1-\intensity,1}
                \fi
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        \foreach \x/\year/\text in {
            ...
            \node[label, above] at (\x, 0.8) {\year\\[1pt]\textbf{\text}};
        }
        ...
        \foreach \x/\year/\text in {
            ...
            \draw[thick] (\x, 0) -- (\x, -0.6);
            \node[label, below] at (\x, -0.8) {\year\\[1pt]\textbf{\text}};
        }
    \end{tikzpicture}
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        % Abstract class
        \node[class, fill=yellow!10] (shape) at (0, 3) {
