...
prevalent in \LaTeX{}, since it itself is so limited.%anot
\footnote{
...
\begin{center}
  \href{https://collaborating.tuhh.de/m21/public/theses/latex_dockerfile}{\LaTeX{} Dockerfile of the ITT}.
\end{center}
...
With Docker installed, you can decide between two opportunities:
\begin{enumerate}
  \item Using Docker itself to compile
  \item Using \href{ch:vsc}{Visual Studio Code} to compile (recommended for beginners, see \cref{ch:editor})
\end{enumerate}
In this \lcnamecref{ch:using-docker} the first opportunity will be explained and in \cref{ch:compilation_steps} the
different compilation methods will be explained. If you want to use \href{ch:vsc}{Visual Studio Code} you should just read \cref{ch:compilation_steps} to understand
the different compilation methods, but don't worry about the terminal Code. The rest can be skipped and you proceed with \cref{ch:editor}.
...
\begin{itemize}
  ...
  \item The final argument to \texttt{docker run} is the image to be run, in this case
        \path{ghcr.io/wyattau/omnilatex-docker:latest}, which will use the image specified by the Link.
\end{itemize}
...
you \texttt{git push} to the remote server (or whenever you configure it to). For more
information have a look at \cref{ch:GitLab}. It does not get simpler; the downside is of course the
lengthier setup, but all of that is explained in the
...
\subsubsection{Complete (\emph{slow}) compilation}
...
\begin{enumerate}
  ...
  \item A \texttt{test.aux} file (and probably others, \iecfeg{e.g.}\ \texttt{.toc}) will have been generated, as well as the \abb{portable_document_format} file itself.
        ...
        The \abb{portable_document_format} has literal \verb|??| markers where there should be references.
   ...
   \item Hence, you compile again.
        ...
        The \verb|??| markers are now replaced by their properly spelled\-/out reference.
        However, what if, hypothetically, one of these \verb|??| substitutions now reads \emph{very super long reference to page 37 in the document}.
...
What if you do not care if there are \verb|??| markers left, undefined entries, shifted pages and so forth, maybe because you are only drafting things up?
...
It does, as opposed to \texttt{latexmk}, not \enquote{just work} anymore, and beginners might wonder what those \verb|??| are.
Now that you know, you may handle them.

\paragraph{Faster (\emph{incomplete}) compilation}
The base \texttt{docker} commands were shown at the beginning of \cref{ch:using-docker}.
An alternative is to run
\begin{minted}[linenos=false,style=default]{shell}
    docker run --rm --volume $(pwd):/tex https://collaborating.tuhh.de/m21/public/theses/latex_dockerfile -e '$max_repeat=1'
\end{minted}
...
\paragraph{Fastest (\emph{incompletest}) compilation}
...
\begin{minted}[linenos=false]{shell}
    docker run --rm --volume $(pwd):/tex --entrypoint="lualatex" https://collaborating.tuhh.de/m21/public/theses/latex_dockerfile --shell-escape $FILENAME
\end{minted}
...
\paragraph{Print version}
There was a time of this \LaTeX{} document where the print version was compiled separately, with an own \LaTeX{} Workshop recipe or an own GitLab pipeline job.
Since we now use the \ctanpackage{ocgx2}-package (as addon for \ctanpackage{hyperref}), which colors links in the \abb{portable_document_format} file, but doesn't print them, this is no longer necessary.
...
\begin{enumerate}
  \item A \textbf{\texttt{Dockerfile} text document} is created, containing instructions on how the image should look like (like what stuff to install, what to copy where, ...).
        ...
        \begin{center}
          \url{https://collaborating.tuhh.de/m21/public/theses/latex_dockerfile}
        \end{center}
        ...
        \begin{minted}{dockerfile}
            FROM https://collaborating.tuhh.de/m21/public/theses/latex\_dockerfile
  ...
  \item The \textbf{image} is then built according to the \texttt{Dockerfile} instructions,
        ...
        be on the same page (alternatively, you could build the image from the Dockerfile, but this feature is
        deprecated/outsourced to the \href{https://collaborating.tuhh.de/m21/public/theses/latex_dockerfile}{LaTeX{} Dockerfile Repo}).
        Because the Dockerfile this project uses can not be found on Dockerhub the full path/link to it has to be given.
...
\begin{enumerate}
  ...
  \item Run \menu{LaTeX Workshop: Build with recipe > Complete compilation}.
\end{enumerate}
This just runs \texttt{latexmk} within the container. Or you can click TEX on the left side of VSC and under Build LaTeX project select \menu{Complete compilation}.
...
With the new recipes, you can now run all three in \cref{ch:compilation_steps} described compilation methods:
\begin{itemize}
  \item \menu{LaTeX Workshop: Build with recipe > Complete compilation}, or
  \item \menu{LaTeX Workshop: Build with recipe > Faster compilation}, or
...
It is highly recommended to use this cookbook in combination with GitLab. Therefore,
you just fork the GitLab Repo of this cookbook. Then you just have to install git
and pull this Repo afterwards. If you do so, you have many nice features enabled
from which you can profit. For example everytime you push a new version to GitLab,
the CI pipeline compiles the whole document for you.
And of course, like this you have a version control and backup of your document.
So, please consider using GitLab.
