...
\begin{table}[tbp]
    ...
    \begin{tabular}{
        ...
        \bottomrule
    \end{tabular}
    \omnlFloatFoottext[1]{Absolute value}
    \omnlFloatFoottext[2]{Time / positional derivative}
...
\begin{table}[tbp]
    ...
    \begin{tabular}{
        ...
        \bottomrule
    \end{tabular}
    \caption[Sans-Serif Examples]{Examples for font features offered by the sans-serif font}
...
\begin{table}[tbp]
    \caption{%
        ...
        Note how the commands are language\-/dependent; no manual adjustments necessary%
    }
    \label{tab:languages}
    \centering
    \footnotesize
    \begin{tabular}{
        ...
        \parbox{0.15\linewidth}{\centering\begin{english}\MakeUppercase{Header}\end{english}}          &
        \parbox{0.25\linewidth}{\centering\begin{english}\hyphenation{read-ing}\end{english}}                                                                    \\
        German                                                                                         &
        \begin{german}\enquote{Zitat!}\end{german}                                                     &
        \begin{german}\qty{100,2}{\meter}\end{german}                                                  &
        \parbox{0.15\linewidth}{\centering\begin{german}\MakeUppercase{\"Uber\"erschrift}\end{german}} &
        \parbox{0.25\linewidth}{\centering\begin{german}\hyphenation{Zu-sam-men-ar-beit}\end{german}}                                                            \\
        \bottomrule
    \end{tabular}
\end{table}
...
In this document, the use of citations is facilitated by \ctanpackage{biblatex} and its backend \ctanpackage{biber}, with \ctanpackage{bibtex} considered outdated. \ctanpackage{biblatex} is recommended for its UTF-8 support, essential for citing authors with international names.

For reliable source citation without worrying about specific citation styles (chicago, apa, etc.), use the command \fakeverb{\cite{bibid}}. This command adapts globally to the desired citation style and is correctly defined for your thesis. For detailed examples of various citation commands and examples, see Table \ref{tab:citationcommands}.

Citations can be placed in the text in various ways:
\begin{itemize}
    \item Directly in the text with \fakeverb{\textcite{bibid}}
    \item In brackets at the end of a sentence or paragraph with \fakeverb{\cite{bibid}}
...
\begin{landscape}
    ...
    \begin{longtblr}[
        ...
        \textcquote{einstein1905}{quote} & \textcquote{einstein1905}{quote} & Einstein remarked: \textcquote{einstein1905}{Mass and energy are equivalent.}.                                         \\
...
Note the block\-/like \texttt{itemize} symbol (\raisebox{0.2ex}{\rule{0.7ex}{0.7ex}})%
\footnote{%
    This is also a regular Unicode character (U+25AA, \texttt{BLACK SMALL SQUARE}).
...
\textbf{Abbreviations} are displayed in the long form, when they are first used, with the short form in brackets.
After that, they are always used in the short form. In this first call
\abb{comprehensive_tex_archive_network} is written out. Now, in it's second call
the short form is used: \abb{comprehensive_tex_archive_network}. Although this is very
...
\glsxtrlong{abb.comprehensive_tex_archive_network}.
...
\begin{table}[tbp]
    ...
    \begin{tabular}{%
        ...
        \addlinespace
        \texttt{constants}\footnotemark[1] &
...
As you can see, even glossary entries with german umlauts work: \abb{wärmepumpe}.
And if it is used the second time (like here: \abb{wärmepumpe}) the short form is used.
