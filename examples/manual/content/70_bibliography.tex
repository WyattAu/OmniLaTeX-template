...
\begin{longtblr}[
    ...
    Command              & Output \\
    \midrule
    \texttt{\textbackslash\{\}cite\{key\}}        & Numeric label: [1] \\
    \texttt{\textbackslash\{\}textcite\{key\}}    & Author (year) inline \\
    \texttt{\textbackslash\{\}parencite\{key\}}   & Parenthesised: (Author, year) \\
    \texttt{\textbackslash\{\}footcite\{key\}}    & Footnote citation \\
    \texttt{\textbackslash\{\}fullcite\{key\}}    & Full bibliographic entry inline \\
    \texttt{\textbackslash\{\}autocite\{key\}}    & Context-sensitive (parencite or footcite) \\
    \texttt{\textbackslash\{\}citeauthor\{key\}}  & Author name only \\
    \texttt{\textbackslash\{\}citeyear\{key\}}    & Year only \\
    \texttt{\textbackslash\{\}parencites\{key1\}\{text1\}} & Multiple parenthesised citations \\
    \texttt{\textbackslash\{\}textcites\{key1\}\{text1\}}  & Multiple textual citations \\
    \texttt{\textbackslash\{\}cites\{key1\}\{text1\}}      & Multiple citations with notes \\
    \texttt{\textbackslash\{\}footcites\{key1\}\{text1\}}  & Multiple footnote citations \\
    \bottomrule
\end{longtblr}
...
\end{verbatim}
    ...
    \item[{\texttt{refsegment}}] -- Segments share a single bibliography but
        group entries by segment. Use \texttt{\textbackslash printbibliography[%
        segment=\textbackslash therefsegment]} to list only the current
        segment's entries.
\end{description}
